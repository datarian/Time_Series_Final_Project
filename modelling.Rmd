...
The missing values are restricted to  be spread equally over the time series by defining 40 blocks, the first beeing from the year 1401 to 1409, the second from year 1411 to 1419, and so forth. This seletion method guarantees to only have one consecutive year missing. From each block, we randomly choose one value as missing. The unknown standard deviation, needed for the backtransformation, is either chosen (i) from an ARMA(1,1) (model selection) process or (ii) linearly interpolated. We also compare the results to the case where we know the standard deviation (see Table \ref{tab:modelcomp}) and to the case where the imputed value is obtained by linear interpolation on the original time series. The coefficients of the linear model needed for the backtransformation are estimated each time without knowing the the missing value.


Assuming an ARMA(1,1) process for the transformed tree ring width series does not outperform the case where we linearly interpolate the same series. Not surprisingly, the best results are obtained when the standard deviation is known. However, simple linear interpolation on the original time series performs poorly compared to the other approaches with an MSE of 302.4. The results of the imputation are plotted in Figure \ref{fig:imputationcomparison}.
...
The `na.kalman` method of the `imputeTS` package, for which ARMA(1,1) processes are assumed for the transformed times series and for the standard deviation by automatic model selection using `auto.arima` from the `forecast` package, leads to similar results (see Figure \ref{fig:imputationcomparison}). Comparing the values of our implementation versus `na.kalman` gives an MSE of $\text{8.8}\cdot \text{10}^\text{5}$ for the transformed series and a MSE of 2.7 in case of the standard deviation. However, in an overall comparison the `na.kalman` performs better than our implementation by achieving an MSE of 256.1 when assuming an ARMA(1,1) for the transformed time series and the standard deviation, and equally when the standard deviation is linearly interpolated. The differences may be explained by our implementation of the ARMA(1,1), where we have to add some 'noise' to the covariance matrix of the state equation in order to be able to estimate the model with the `dlm` package. 
