...
Section [3.2](#modelselection) gives evidence that the ARMA(1,1) model describes the stochastic process best. We expect to see the same pattern when we compare the models with respect to their imputation performance. Table \ref{tab:modelcomp} shows the MSE (mean squared errors) for the stationary series denoted as "transformed", and the backtransformed series denoted as "original". The MSE are the results of a small simulation study, where we set 40 times randomly one observation as missing.

The missing values are restricted to  be spread equally over the time series by defining 40 blocks, the first beeing from the year 1401 to 1409, the second from year 1411 to 1419, and so forth. From each block, we randomly choose one value as missing. The unknown standard deviation, needed for the backtransformation, is either chosen (i) from an ARMA(1,1) (model selection) process or (ii) linearly interpolated. We also compare the results to the case where we know the standard deviation (see Table \ref{tab:modelcomp}) and to the case where the imputed value is obtained by linear interpolation on the original time series. The coefficients of the linear model needed for the backtransformation are estimated each time without knowing the the missing value.

On the transformed series, ARMA(1,1) performs best with an MSE of 0.005. However, simple linear interpolation does the job almost as accurately. A similar pattern is seen on the original series. Assuming an ARMA(1,1) does not really outperform linear interpolation. Not surprisingly, the best results are obtained when the standard deviation is known. However, simple linear interpolation on the original time series performs poorly compared to the other approaches with an MSE of 302.4. The results of the imputation are plotted in Figure \ref{fig:imputationcomparison}.
...
The `na.kalman` method of the `imputeTS` package, for which ARMA(1,1) processes are assumed for the transformed times series and for the standard deviation by automatic model selection using `auto.arima` from the `forecast` package, leads to similar results. Comparing the values of our implementation versus `na.kalman` gives an MSE of $\text{8.8}\cdot \text{10}^\text{5}$ for the transformed series and a MSE of 2.7 in case of the standard deviation. However, in an overall comparison the `na.kalman` performs better than our implementation by achieving an MSE of 256.1 when assuming an ARMA(1,1) for the transformed time series and the standard deviation, and equally when the standard deviation is linearly interpolated (own implementation: 274.2 vs. `na.kalman`: 275.7). The differences may be explained by our implementation of the ARMA(1,1), where we have to add some 'noise' to the covariance matrix of the state equation in order to be able to estimate the model with the `dlm` package. 
