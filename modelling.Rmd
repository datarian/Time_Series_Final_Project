...
The validity of all models was assessed graphically by plotting the standardized residuals, residual ACF and Ljung-Box test for independence for all three models combined.

For the ARMA(1,1) model, the residuals are confirmed to be white noise (see \ref{fig:tsdiag}. The Ljung-box statistic for various lags showed that for the AR(1) and AR(2) model, the residuals are not white noise (see \ref{fig:tsdiag.ar1}, which is also evident from the ACF, which shows some dependency is left.

```{r tsdiag, fig.cap="Diagnostic plots for ARMA(1,1), AR(1) and AR(2). Top: Standardized residuals, middle: ACF, bottom: Ljung-Box test", fig.asp=1.5, out.width="0.5\\linewidth"}
grid.arrange(
    residuals_plot,
    acf_comparison_plot,
    boxtest_plot,
