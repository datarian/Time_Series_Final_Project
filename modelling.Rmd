...
On our specific dataset, a combination of scaling, log-transforming and subtracting a linear trend produced the best result in our view, yielding a times series that looks reasonably (weakly) stationary as the Figure \ref{fig:stationarity} visually assures. The series is mean-centered and shows a relatively constant variance. However, two outliers, which were already evident in the original data, remain. A possible explanation for extreme values for $\bar{r}_t$ with a small $sd_t$ for the same $t$ could be volcanic activity, which has been shown to impact tree growth [@sigl2015timing]. But this would only explain the negative outlier (small ring width). The results of the Box-Cox, the Power transformation and the method of Warren (1980) were not satisfactory. The methods lead to a transformed times series with a slowly decreasing ACF, which let us doubt whether the resulting time series is indeed stationary.

Stationarity was only assessed graphically. Unit root tests such as the Augemented Dickey-Fuller (ADF) or Phillips-Perron (PP) test, or other tests for stationarity like Kwiatkowski-Phillips-Schmidt-Shin (KPSS) test have usually very limitied explanatory power due to their sensitivity on assumptions and are thus not performed.

The ACF and PACF for the stationary series (see Figure \ref{fig:acfpacf_chosen}) give further evidence for stationarity. The other three approaches discussed above  showed a slow ACF decrease.^[The ACF of a random walk shows such a pattern. However, a random walk is not stationary and thus needs to be further transformed, usually by taking the difference $x_{t+1} - x_t$. In case of a random walk the resulting time series is white noise.] For the sake of brevity these results are not shown.
...
Despite the fact that ARMA(1,1) is the best model in terms of AIC, we decided to include AR(1), AR(2) and ARMA(1,1) in our analysis because all of them are frequently found in the literature to fit dendrochronological data well. Table \ref{tab:modelaic} lists values of the AIC for the different models. ARMA(1,1) shows the best fit to the data, followed by the AR(2) model. The performance of the AR(1) model is the weakest. In Table \ref{tab:modelcomp} the estimated parameters for all models are presented. The parameters are significantly different from 0 at an $\alpha$-level of 5\% for any model [DA LIEGT ABER KEIN TEST ZUGRUNDE. HAB ICH EINFACH SO GEMACHT, WEIL CLéMENT DAS SO GEZEIGT HAT IN DER VORLESUNG].
...
Section [3.2](#modelselection) gives evidence that the ARMA(1,1) model describes the stochastic process best. We expect to see the same pattern when we compare the models with respect to their imputation performance. Figure \ref{tab:modelcomp} shows the MSEs (mean squared errors) for the stationary series denoted as "transformed", and the backtransformed series denoted as "original". The MSE are the results of a small simulation study, where we randomly set 40 times one observation as missing.

The missing values are restricted to  be spread equally over the time series by defining 40 blocks, the first beeing from 1401 to 1409, the second from 1411 to 1419, and so forth. From each block, we randomly choose one value as missing. The unknown standard deviation, needed for the backtransformation, is chosen (i) from an ARMA(1,1) (model selection), (ii) is linearly interpolated or  (iii) taken from the actually known values before deleting observations [STIMMT DAS SO?]. The coefficients of the linear model needed for the backtransformation are estimated on the data with missing values.

On the transformed series, ARMA(1,1) performs best with an MSE of 0.005. However, simple linear interpolation does the job almost as accurately. A similar pattern is seen on the original series. Assuming an ARMA(1,1) does not really outperform linear interpolation. Not surprisingly, the best results are obtained when the standard deviation is known. Simple linear interpolation on the original time series performs poorly compared to the other approaches with an MSE of 302.4. The results of the imputation are plotted in Figure \ref{fig:imputationcomparison}.
...
The `na.kalman` method of the `imputeTS` package, for which ARMA(1,1) processes are assumed for the transformed times series and for the standard deviation by automatic model selection using `auto.arima` from the `forecast` package, leads to similar results. Comparing the values of our implementation versus `na.kalman` gives an MSE of $\text{8.8}\cdot \text{10}^\text{5}$ for the transformed series and an MSE of $2.7$ in case of the standard deviation. However, in an overall comparison the `na.kalman` performs better than our implementation by achieving an MSE of $256.1$ when assuming an ARMA(1,1) for the transformed time series and the standard deviation, and equally when the standard deviation is linearly interpolated (own implementation: 274.2 vs. `na.kalman`: 275.7). The differences may be explained by our implementation of the ARMA(1,1), where we have to add some noise to the covariance matrix of the state equation in order to be able to estimate the model with the `dlm` package. 
