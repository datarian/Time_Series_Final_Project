...
model <- lm(spruce_sub900_ts~t+t_sq+t_cu)
...
r1d1 <- resid_spruce_sub900[1:(length(resid_spruce_sub900)-1)]
ar1d2 <- resid_spruce_sub900[-1]
plot(ar1d2~ar1d1)
ar1_model <- lm(ar1d2~ar1d1)
summary(ar1_model) # 
acf(ar_model$residuals,lag.max = 200) # already looks a lot better, but to many gamma(h) outside of range
